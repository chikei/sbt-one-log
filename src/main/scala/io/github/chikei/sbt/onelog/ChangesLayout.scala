...
object ChangesLayout {
  ...
  def ascii(changes: Updates, renderAdd: ModuleId => String, renderOverride: ModuleId => String): String = {
  ...
  private def excludes(ex: Map[ModuleId, Set[ModuleId]]): String = {
  ...
  private def addes(add: Set[ModuleId], render: ModuleId => String): String = {
    add.toSeq.sorted.map(a =>
      twoSpace + "libraryDependencies += " + render(a)
    ).mkString("\n")
  }

  private def overrides(over: Map[ModuleId, String], render: ModuleId => String): String = {
